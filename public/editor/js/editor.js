		...
		createMenuItem: function(itemSettings, options, returnElement){
			...
			if(itemSettings["select"]){
				...
				menuWrapElement.append($('<a/>',{
										class:"btn botonH dropdown-toggle",
										"data-toggle":"dropdown",
										"href":"javascript:void(0)",
										"title":itemSettings["tooltip"]
										}).html(itemSettings["default"]).append($("<span/>",{class:""})).mousedown(function(e){
											e.preventDefault();
										}));
				...
				var menuWrapElement = $("<a/>",{href:'javascript:void(0)', class:'btn botonH'});
				var menuElement = $("<i/>");
				if(itemSettings["icon"])
					menuElement.addClass(itemSettings["icon"]);
				else
